...
-- inner join
select * from book join status on status_id = status.id where species='cat';
select * from status join book on status_id = status.id where species='cat';
select * from book join status on pet.status_id = status.id where species='cat' order by pet.name;
select * from book join status on pet.status_id = status.id where species='cat' order by pet.name desc;

-- outer joins
select * from book full outer join status on pet.status_id=status.id;
select * from book right join status on pet.status_id=status.id;
select * from book left join status on pet.status_id=status.id;
